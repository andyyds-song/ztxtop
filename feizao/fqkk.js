    ...
    try {
      ...
      if (ac.rest) {
        if (ac.rest <= 0 || (fqkkBanfirstTask && ac.num <= 0)) {
          ...
          $.log(`😄账号${ac.no} 今日已阅读${ac.num}次，本阶段待阅读${ac.rest}次，即将阅读\n`);
          await $.wait((i + 1) * 600)
          let flag = 0;
          let count = 1;
...
function getExecAcList() {
  ...
  $.log(`番茄看看当前设置的提现金额为: ${fqtx / 100} 元`, `----------- 共${len}个账号分${execAcList.length}组去执行 -----------`)
  return execAcList
}
        ...
        try {
          if (err) {
            ...
            if (result.code == 0 && result.data && result.data.jkey) {
              ...
              if (jumpObj) {
                let time = parseInt(Math.random() * (12 - 9 + 1) + 9, 10)
