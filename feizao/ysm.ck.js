...
const $ = new Env('云扫码CK刷新')
let ysm = $.getjson('ysm', [])

let format = 0 // 存储的json数据是否需要美化(格式化)：0-不美化、1-美化

!(async () => {
  if (typeof $request == "undefined") {
    let acList = ysm.filter(o => o.openid).map((o, i) => ({...{no: i + 1}, ...o}))
    $.log(`----------- 共${acList.length}个账号待更新 -----------`)
    let idMap = {}
    let hostList = []
    for (let ac of acList) {
      if (($.time('HH') - 0 + ac.no) % acList.length == 0) {
        let newAc = await updateAndGetCk(ac)
        if (newAc) {
          hostList.push(`【账号${ac.no}】\n旧Origin：${ac.domain}\n新Origin：${newAc.domain}`)
          idMap[newAc.openid] = newAc
        }
        await $.wait(1000)
      }
    }
    for (let ac of ysm) {
      let newAc = idMap[ac.openid]
      if (newAc) {
        Object.assign(ac, newAc)
      }
    }
    if (hostList.length > 0) {
      if(format){
        $.setdata(JSON.stringify(ysm), 'ysm')
      } else {
        $.setdata(JSON.stringify(ysm, null, 2), 'ysm')
      }
      $.log('======== [begin] ========', hostList.join('\n\n'), '======== [end] ========')
      $.msg($.name, "", `云扫码更新ck数据成功！`)
    } else {
      $.log('\n======== [无数据更新] ========\n')
    }
  }
...
function updateAndGetCk(ac) {
  return new Promise(resolve => {
    let opts = {
      url: `${ac.domain}redirect/${ac.secret}?openid=${ac.openid}`
    }
